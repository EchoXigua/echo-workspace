...
V1.1 不做复杂离线同步，但游客模式需要支持本地正式饮食记录。

需要支持：

- 手动饮食草稿。
- 游客未登录时保存的本地 confirmed 饮食记录。
- 体重记录提交失败后的待同步。
- AI 识别失败后保留用户输入。

数据链路约定：

- 登录态：添加、编辑、删除饮食记录走后端接口，首页以后端返回为准。
- 游客态：添加饮食记录写入 `LocalStore`，首页基于本地记录即时更新。
- 游客登录后：客户端调用 `POST /v1/diet/entries/sync-local` 把本地未同步饮食记录批量导入后端，成功后标记本地记录已同步或清理本地游客数据。

登录后后端仍是主数据源。重新进入页面时优先拉后端数据，再合并必要的本地草稿或待同步项。

当前实现：

- `FileLocalStore`：真实 App 环境默认使用。
- `InMemoryLocalStore`：Mock、预览和测试使用。
- `LocalStore` 需要区分本地草稿和游客本地 confirmed 记录；后者不是临时草稿，会驱动游客首页统计。
