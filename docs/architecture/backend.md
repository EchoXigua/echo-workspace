...
V1.1 优先保证在线体验，但首次进入允许游客模式轻量使用。数据链路分两类：

- 登录态：饮食记录、体重记录、首页统计都以后端为准，客户端直接调用 API。
- 游客态：饮食记录可以先保存到客户端本地正式记录区，首页基于本地记录即时更新。

客户端可以做本地草稿、游客本地正式记录和失败重试：

- 手动记录失败时保留本地草稿。
- 拍照识别失败时允许改为手动录入。
- 体重记录失败时进入待同步队列。
- 游客未登录时保存的饮食记录保存在本地；用户后续登录后，客户端通过 `POST /v1/diet/entries/sync-local` 批量同步到后端。

登录后跨端一致性以后端数据为准。V1.1 不做复杂离线同步冲突解决；游客本地记录同步按 `clientLocalId` 幂等导入，避免重试重复写入。
