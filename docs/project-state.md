...
iOS 基础设施阶段已完成：SwiftUI 工程、MVVM + feature-first 目录、API DTO、Live/Mock API Client、Token 存储、本地草稿存储边界、设计系统、可复用组件、设计稿画面映射和基础单元测试 Target 已初始化。iOS App 构建和 build-for-testing 已通过；业务开发尚未开始，等待用户确认后再启动。
...
2. iOS 业务开发需等待用户确认；确认后读取 `ios/docs/infrastructure-status.md`、`ios/docs/design-screen-map.md` 和 `docs/product/v1.1-acceptance-criteria.md`，先在本机模拟器补跑 `xcodebuild test`，再从 Onboarding / 登录占位和用户档案填写开始。
...
```text
请读取 AGENTS.md、ios/AGENTS.md、docs/project-state.md、ios/docs/development-readiness.md、ios/docs/infrastructure-status.md、ios/docs/design-screen-map.md 和 docs/product/v1.1-acceptance-criteria.md，继续 LeanMate V1.1 iOS。iOS 基础设施、设计稿画面映射和基础单元测试 Target 已完成；App 构建和 build-for-testing 已通过，业务开发尚未开始。除非我明确确认进入业务开发阶段，否则不要实现业务页面。进入业务开发后，必须复用现有 APIClient、MockAPIClient、设计系统和 Components，并且只以 design/app/LeanMateV1.0-shikaka.pen 为设计准稿，不要实现超出 V1.1 PRD、验收标准和 OpenAPI 的功能。
```
