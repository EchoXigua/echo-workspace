...
- AI 日报和连续打卡业务 API 尚未实现；iOS 首页和我的页当前只展示后端返回的 streak 或 Mock 数据，不在客户端自行计算连续打卡。
...
- 对象存储真实配置尚未确认。
- 图片保存期限和隐私策略尚未确认。
- iOS Onboarding、ProfileSetup、Home、Diet 文本/手动记录入口、Diet 拍照识别确认、Diet 删除确认、Weight 记录 Sheet、AI 日报入口、Profile Summary / 我的页、连续打卡展示和里程碑弹窗已完成。AI 日报页和连续打卡当前按 OpenAPI 和 MockAPIClient 做状态流，真实数据依赖后端日报与 retention 接口完成。
- iOS Bundle ID、Apple Developer Team、真实登录能力尚未确认。
- iOS 当前本地持久化使用 `FileLocalStore`；如业务阶段确需 SwiftData，可替换 `LocalStore` 实现。
- 当前沙箱无法访问具名 iOS Simulator 执行真实 `xcodebuild test`；第 1、2、3、4、5 批均已通过 generic simulator `xcodebuild build` 和 `build-for-testing`，进入联调前建议在本机模拟器补跑真实 `test`。

## Recommended Next Steps

1. 后端新对话：读取 `server/docs/development-readiness.md`、`docs/api/openapi.yaml`、`server/docs/technical-design/v1.1/05-ai-daily-report.md` 和 `server/docs/ai-provider.md`，从 AI 日报接口继续实现业务 API。
2. 后端 Retention 批次：实现 `GET /v1/retention/streak` 的真实连续打卡口径，客户端只展示后端返回值。
3. iOS 联调收口批次：接真实后端返回做端到端验收、补跑具名模拟器 `xcodebuild test`，不新增 PRD 之外功能。
...
```text
请读取 AGENTS.md、ios/AGENTS.md、.codex/skills/leanmate-batch-coding-workflow/SKILL.md、docs/project-state.md、ios/docs/development-readiness.md、ios/docs/infrastructure-status.md、ios/docs/design-screen-map.md、ios/docs/v1.1-state-matrix.md、docs/product/v1.1-acceptance-criteria.md、docs/api/openapi.yaml 和 docs/product/prd-v1.1.md，开始 LeanMate V1.1 iOS 联调收口批次。当前 iOS 第 1-5 批业务页面已完成：Onboarding/ProfileSetup/Home/Diet/Weight/Report/Profile Summary/Retention 展示。请先检查 `git status --short`，只做真实后端联调、状态回归、构建测试和必要的小范围修复。必须复用现有 APIClient、MockAPIClient、AppRouter、设计系统和 Components，只以 design/app/LeanMateV1.0-shikaka.pen 为设计准稿。不要实现真实 Apple 登录，不要在客户端自行计算 streak，不要实现超出 V1.1 PRD、OpenAPI、验收标准和状态矩阵的功能。完成后运行 xcodebuild build、build-for-testing，并在具名模拟器可用时补跑 xcodebuild test；不要自动提交，除非用户明确要求。
```
