...
- 后端通用响应、错误码、异常处理、参数校验错误响应、CORS、JWT 鉴权、当前用户上下文、认证接口、当前用户接口、用户档案接口、体重记录接口、首页今日状态接口、手动饮食记录接口、饮食 AI 识别任务接口和 AI 日报接口已实现。
- 连续打卡业务 API 尚未实现；iOS 首页和我的页当前只展示后端返回的 streak 或 Mock 数据，不在客户端自行计算连续打卡。
...
- 对象存储真实配置尚未确认。
- 图片保存期限和隐私策略尚未确认。
- iOS Onboarding、ProfileSetup、Home、Diet 文本/手动记录入口、Diet 拍照识别确认、Diet 删除确认、Weight 记录 Sheet、AI 日报入口、Profile Summary / 我的页、连续打卡展示和里程碑弹窗已完成。AI 日报页需要接已完成的后端日报接口做真实联调；连续打卡真实数据依赖后端 retention 接口完成。
...
1. 后端新对话：读取 `server/docs/development-readiness.md`、`docs/api/openapi.yaml` 和 `server/docs/technical-design/v1.1/04-home-stats-streak.md`，从 Retention 批次继续实现 `GET /v1/retention/streak` 的真实连续打卡口径，客户端只展示后端返回值。
...
```text
请读取 AGENTS.md、server/AGENTS.md、docs/project-state.md、server/docs/development-readiness.md、docs/api/openapi.yaml 和 server/docs/technical-design/v1.1/04-home-stats-streak.md，继续 LeanMate V1.1 后端编码阶段。后端 Spring Boot 3.x + Java 17 + Maven 工程骨架、Flyway 迁移脚本、基础设施、认证接口、当前用户接口、用户档案接口、体重记录接口、首页今日状态接口、手动饮食记录接口、饮食 AI 识别任务接口和 AI 日报接口已完成并通过离线测试，下一步实现 `GET /v1/retention/streak` 的真实连续打卡接口，不要实现超出 V1.1 文档范围的功能。
```
