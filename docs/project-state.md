...
2026-06-11 已新增 iOS Debug 本地调试登录入口：在 Debug 构建且 live API baseURL 为本机地址时，欢迎页显示“本地调试登录”按钮；按钮调用现有 `POST /v1/auth/oauth-login`，仅 Apple identity token 校验走后端 local/test `mock:` verifier，账号创建、JWT、refresh token 和后续业务接口均走真实后端与真实数据库。

2026-06-11 已完成一次 iOS V1.1 真实后端业务自测，报告见 `docs/qa/ios-v1.1-self-test-2026-06-11.md`。登录、档案、首页、手动饮食、体重、日报、我的页和删除回算主链路可跑通；新增待处理问题包括文本识别确认页保存 0 kcal、access token 过期后我的页首次加载未知错误、体重 Sheet 默认值未取用户当前/最近体重、饮食保存成功页出现“暂无内容”、首页缺少饮食历史/详情/编辑入口，以及本地 health endpoint 被鉴权拦截。
...
- AI agent 上下文启动必须先定位真实 Git 根目录并读取根目录 `AGENTS.md` 与任务涉及子目录 `AGENTS.md`。本项目可能从外层目录 `/Users/edy/Documents/瘦搭` 启动，真实 repo 在 `echo-workspace` 时不能假设项目规则已自动进入上下文。提交代码、整理提交或检查 commit message 前，必须显式读取 `.codex/skills/leanmate-git-commit/SKILL.md`，再按中文 Conventional Commits 规范提交。
...
- iOS Debug 本地调试登录入口已实现，用于在 Apple 登录生产配置未就绪时走通真实后端联调；该入口只在 Debug + 本机 live API 下显示，固定使用 `mock:ios-local-user:ios-local@leanmate.local`。
- 对象存储真实配置尚未确认。
- 图片保存期限和隐私策略尚未确认。
