...
- 对象存储真实配置尚未确认。
- 图片保存期限和隐私策略尚未确认。
- iOS Onboarding、ProfileSetup、Home、Diet 文本/手动记录入口、Diet 拍照识别确认、Diet 删除确认、Weight 记录 Sheet 和 AI 日报入口已完成；Profile Summary 和 Retention 业务页面尚未实现。AI 日报页当前按 OpenAPI 和 MockAPIClient 做状态流，真实数据依赖后端日报接口完成。
- iOS Bundle ID、Apple Developer Team、真实登录能力尚未确认。
- iOS 当前本地持久化使用 `FileLocalStore`；如业务阶段确需 SwiftData，可替换 `LocalStore` 实现。
- 当前沙箱无法访问具名 iOS Simulator 执行真实 `xcodebuild test`；第 1、2、3、4 批均已通过 generic simulator `xcodebuild build` 和 `build-for-testing`，进入后续批次或联调前建议在本机模拟器补跑真实 `test`。

## Recommended Next Steps

1. iOS 第 5 批子线程：实现 Profile Summary / 我的页、连续打卡展示和里程碑弹窗；客户端只展示后端返回的 streak，不自行计算。
2. 后端新对话：读取 `server/docs/development-readiness.md`、`docs/api/openapi.yaml`、`server/docs/technical-design/v1.1/05-ai-daily-report.md` 和 `server/docs/ai-provider.md`，从 AI 日报接口继续实现业务 API。
3. 确认 AI Provider、模型名、Apple 登录、对象存储和 iOS Bundle ID 后更新相关文档，并进入真实后端联调。
...
```text
请读取 AGENTS.md、ios/AGENTS.md、.codex/skills/leanmate-batch-coding-workflow/SKILL.md、docs/project-state.md、ios/docs/development-readiness.md、ios/docs/infrastructure-status.md、ios/docs/design-screen-map.md、ios/docs/v1.1-state-matrix.md、docs/product/v1.1-acceptance-criteria.md、docs/api/openapi.yaml 和 docs/product/prd-v1.1.md，开始 LeanMate V1.1 iOS 第 5 批业务开发。当前 iOS 第 1 批已完成并提交 `dd3f3f1 feat(ios): 实现 V1.1 首批登录档案流程`，第 2 批 Home 已完成并提交 `65261ac feat(ios): 实现 V1.1 首页业务流程`，第 3 批 Diet / Weight 已完成并提交 `3bc6216 feat(ios): 实现 V1.1 饮食和体重记录入口`，第 4 批拍照识别确认、删除确认和 AI 日报入口已完成。第 5 批请先检查 `git status --short`，范围为 Profile Summary / 我的页、连续打卡展示和里程碑弹窗。必须复用现有 APIClient、MockAPIClient、AppRouter、设计系统和 Components，只以 design/app/LeanMateV1.0-shikaka.pen 为设计准稿。不要实现真实 Apple 登录，不要在客户端自行计算 streak，不要实现超出 V1.1 PRD、OpenAPI、验收标准和状态矩阵的功能。完成后运行 xcodebuild build 和 build-for-testing；不要自动提交，除非用户明确要求。
```
