...
iOS 基础设施阶段已完成，V1.1 业务第 1 批和第 2 批已完成并提交。第 1 批完成 Onboarding / 登录占位、ProfileSetup、Mock 登录、档案保存和保存后路由，提交为 `dd3f3f1 feat(ios): 实现 V1.1 首批登录档案流程`。第 2 批完成 Home 首页今日状态、游客态、档案未完成、empty、loaded、error 状态和 HomeViewModel 测试，提交为 `65261ac feat(ios): 实现 V1.1 首页业务流程`。当前主线程只做批次调度和状态落盘，后续 iOS 第 3、4、5 批建议按 `.codex/skills/leanmate-batch-coding-workflow/` 在子线程中开发。
...
- 对象存储真实配置尚未确认。
- 图片保存期限和隐私策略尚未确认。
- iOS Onboarding、ProfileSetup 和 Home 已完成；Diet、Weight、Report、Profile Summary 和 Retention 业务页面尚未实现。
- iOS Bundle ID、Apple Developer Team、真实登录能力尚未确认。
- iOS 当前本地持久化使用 `FileLocalStore`；如业务阶段确需 SwiftData，可替换 `LocalStore` 实现。
- 当前沙箱无法访问具名 iOS Simulator 执行真实 `xcodebuild test`；第 1、2 批均已通过 generic simulator `xcodebuild build` 和 `build-for-testing`，进入后续批次或联调前建议在本机模拟器补跑真实 `test`。

## Recommended Next Steps

1. iOS 第 3 批子线程：按 `.codex/skills/leanmate-batch-coding-workflow/` 启动新子线程，读取 iOS 文档、PRD、验收标准和 OpenAPI，先补 Diet / Weight 状态矩阵，再实现记录入口、手动饮食记录、文本识别入口/确认、体重记录 Sheet 和对应 ViewModel 测试。
2. iOS 第 4 批子线程：实现拍照识别确认、识别任务状态、识别失败转手动编辑、删除确认、AI 日报页面和日报 ViewModel。AI 日报真实联调依赖后端日报接口完成。
...
```text
请读取 AGENTS.md、ios/AGENTS.md、docs/project-state.md、ios/docs/development-readiness.md、ios/docs/infrastructure-status.md、ios/docs/design-screen-map.md、ios/docs/v1.1-state-matrix.md、docs/product/v1.1-acceptance-criteria.md、docs/api/openapi.yaml 和 docs/product/prd-v1.1.md，开始 LeanMate V1.1 iOS 第 3 批业务开发。当前 iOS 第 1 批已完成并提交 `dd3f3f1 feat(ios): 实现 V1.1 首批登录档案流程`，第 2 批 Home 已完成并提交 `65261ac feat(ios): 实现 V1.1 首页业务流程`。本批请先检查 `git status --short`，再按 `ios/docs/v1.1-state-matrix.md` 格式补充 Diet / Weight 状态矩阵，然后实现记录入口、手动饮食记录、文本识别入口/确认、体重记录 Sheet 和对应 ViewModel 测试。必须复用现有 APIClient、MockAPIClient、AppRouter、设计系统和 Components，只以 design/app/LeanMateV1.0-shikaka.pen 为设计准稿。不要实现拍照识别确认、AI 日报详情、我的页、连续打卡里程碑或真实 Apple 登录。完成后运行 xcodebuild build 和 build-for-testing；不要自动提交，除非用户明确要求。
```
