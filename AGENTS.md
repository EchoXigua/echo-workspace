...
当用户要求进入编码环节、按多个批次开发、为下一批创建新对话/派生线程、控制主线程上下文、或要求把批次进度落盘时，先读取项目内批次编码工作流：

- `.codex/skills/leanmate-batch-coding-workflow/SKILL.md`
- `.codex/skills/leanmate-batch-coding-workflow/references/batch-thread-prompts.md`

主线程只保留项目级共享上下文和批次调度；具体编码、调试、构建和提交放到子线程。每个重要编码批次结束前，应更新 `docs/project-state.md` 或对应状态矩阵，并用 Git commit 作为可靠恢复点。
